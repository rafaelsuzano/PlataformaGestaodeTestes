...
const mockCoverageData = [
  { name: 'Cobertos', value: 75, color: '#10b981' },
  { name: 'Não Cobertos', value: 25, color: '#ef4444' },
];

const mockModuleData = [
  { name: 'Autenticação', coberto: 95, total: 100 },
  { name: 'Pagamentos', coberto: 60, total: 100 },
  { name: 'Relatórios', coberto: 40, total: 100 },
  { name: 'Dashboard', coberto: 85, total: 100 },
];

const mockRequirements = [
  { id: 'REQ-001', title: 'Login via SSO', status: 'Alta', coverage: 100 },
  { id: 'REQ-002', title: 'Checkout via PIX', status: 'Crítica', coverage: 50 },
  { id: 'REQ-003', title: 'Exportação PDF', status: 'Média', coverage: 0 },
  { id: 'REQ-004', title: 'Gráficos de Vendas', status: 'Baixa', coverage: 80 },
];
            ...
            <CardContent>
              ...
              <Box sx={{ height: 250 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={mockCoverageData} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
        ...
        <Grid item xs={12} md={7}>
          <Card sx={{ bgcolor: 'rgba(30, 41, 59, 0.6)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 3, height: '100%' }}>
            <CardContent>
              ...
              <Box sx={{ height: 250 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={mockModuleData} layout="vertical" margin={{ top: 0, right: 30, left: 20, bottom: 0 }}>
                  ...
                  <tr key={req.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                    ...
                    <td className="p-3">
                      <Chip 
                        ...
                        size="small" 
                        sx={{ 
                          bgcolor: req.status === 'Crítica' ? 'rgba(239,68,68,0.2)' : req.status === 'Alta' ? 'rgba(245,158,11,0.2)' : 'rgba(52,211,153,0.2)',
                          color: req.status === 'Crítica' ? '#f87171' : req.status === 'Alta' ? '#fbbf24' : '#34d399',
                          fontWeight: 'bold'
                        }} 
                      />
                    </td>
